...
# Mit Sentiment verknüpfen
sentiment_scores <- reviews %>%
  cross_join(bing_sentiments) %>%
...
sentiment_scores %>%
  ...
  coord_flip() +
  labs(title = "Top sentiment words in Disneyland reviews", x = NULL, y = "Anzahl")
...
# 🧹 1. Text vorbereiten: Rezensionstext in Kleinbuchstaben & Sonderzeichen bereinigen
# Annahme: Die Spalte mit Rezensionen heißt 'text'
reviews_tokens <- reviews %>%
  mutate(text = replace_non_ascii(text)) %>%          # ersetzt komische Zeichen
  unnest_tokens(word, text) %>%                       # zerlegt Text in einzelne Wörter
  anti_join(stop_words)                               # entfernt häufige "Füllwörter"

# 📊 2. Wortfrequenz zählen
word_freq <- reviews_tokens %>%
  count(word, sort = TRUE)

# ☁️ 3. Wordcloud erzeugen
wordcloud(words = word_freq$word,
          freq = word_freq$n,
          min.freq = 20,                # nur Wörter mit mindestens 20 Vorkommen
          max.words = 100,              # max. Anzahl anzuzeigender Wörter
          random.order = FALSE,         # häufigste Wörter zentral platzieren
...
```{r}
# Durchschnittliche Bewertung je Standort
reviews %>%
  group_by(Branch) %>%
  ...
  geom_col() +
  coord_flip() +
  labs(title = "Durchschnittliche Bewertung je Disneyland-Standort", x = "Standort", y = "Durchschnittsbewertung")

```

```{r}
monthly_sentiment <- reviews %>%
  cross_join(bing_sentiments) %>%
  count(year, month, sentiment) %>%
  pivot_wider(names_from = sentiment, values_from = n, values_fill = 0) %>%
  mutate(sentiment_score = positive - negative)

ggplot(monthly_sentiment, aes(x = interaction(year, month), y = sentiment_score)) +
  geom_line(group = 1) +
  theme_minimal() +
  labs(title = "Sentiment-Score über Zeit", x = "Monat", y = "Score (Positiv - Negativ)") +
