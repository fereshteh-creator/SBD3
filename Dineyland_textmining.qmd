...
The results show us that there are indeed other languages included in the reviews but we can't make sure that those are correctly detected, so instead of removing them we will make sure to use language models which are multilingual.
...
-   The California park with a strong local customer base likely due to its regional popularity and better experience of so that they tend to go back.
...
3.  Sentiment Classification:\
    Using the HuggingFace Transformers pipeline, each review was passed through the `cardiffnlp/twitter-xlm-roberta-base-sentiment model`. This model was pre-trained on multilingual Twitter data and classifies text into one of three sentiment categories: positive, neutral, or negative.
...
If we have a look at the average true ratings of our sample we notice that Disneyland Paris has the lowest average rating:
...
-   Long wait times, slow food service and closed attractions (time_food_closed_kids)

-   Staff rudeness and smoking staff (smoking_people_smoke_staff)

-   Smoking-related concerns and enforcement (smoking_people_smoke_staff)

-   Toilet cleanliness and hygiene (toilets_disgusting_toilet_euro)

A substantial number of reviews were labeled with Topic -1, which is actually very similar to topic 0, indicating that many of the negative reviews were because of food long queues, staff and closed attractions.

### Frequency of Topics in Negative Disneyland Paris Reviews

Using **BERTopic**, we extracted and categorized common themes in 1,000 negative reviews of **Disneyland Paris**. The chart bellow shows the frequency of each topic, based on semantic clustering of similar reviews.
...
To visulaize the clusters, we applied UMAP (Uniform Manifold Approximation and Projection) to reduce the high-dimensional text embeddings into two dimensions. Each point represents a single review, and colors indicate the topic assigned by the BERTopic model.
...
To help Disneyland in Paris to improve even more lets look at the reasons behind very positive reviews of California
